...
//match the column names to the field names
	if (!empty($delimiter) && file_exists($_SESSION['file']) && $action != 'import') {
		...
		//form to match the fields to the column names
			...
			echo button::create(['type'=>'button','label'=>$text['button-back'],'icon'=>$settings->get('theme', 'button_icon_back'),'id'=>'btn_back','style'=>'margin-right: 15px;','link'=>'contact_import.php']);
...
//show content
	...
	echo "	<div class='heading'><b>".$text['header-contacts_import']."</b></div>\n";
	echo "	<div class='actions'>\n";
	echo button::create(['type'=>'button','label'=>$text['button-back'],'icon'=>$settings->get('theme', 'button_icon_back'),'id'=>'btn_back','style'=>'margin-right: 15px;','link'=>'contacts.php']);
