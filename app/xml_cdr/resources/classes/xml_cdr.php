...
class xml_cdr extends app {
	...
	/**
	 * Saves a log message to the configured logging destination.
	 *
	...
	/**
	 * Defines the fields for the xml_cdr table.
	 *
	 * @return void
	 */
	...
	/**
	 * Saves the call details record to the database and removes old entries in the xml_cdr, xml_cdr_flow, xml_cdr_json, and xml_cdr_logs tables.
	 *
	 * @return void
	 */
	...
	/**
	 * Processes the XML array data and saves it to the database.
	 *
	 * @param string     $key        The key for the XML array.
	 * @param string     $leg        The leg of the call.
	 * @param string     $xml_string The XML string to process.
	 *
	 * @return mixed
	 */
	public function xml_array($key, $leg, $xml_string) {
		...
		if ($duplicate_uuid == false && is_uuid($uuid)) {
			...
			//call center
			if ($xml->variables->cc_member_uuid == '_undef_') {
				$xml->variables->cc_member_uuid = '';
			}
			if ($xml->variables->cc_member_session_uuid == '_undef_') {
				$xml->variables->cc_member_session_uuid = '';
			}
			if ($xml->variables->cc_agent_uuid == '_undef_') {
				$xml->variables->cc_agent_uuid = '';
			}
			if ($xml->variables->call_center_queue_uuid == '_undef_') {
				$xml->variables->call_center_queue_uuid = '';
			}
			if ($xml->variables->cc_queue_joined_epoch == '_undef_') {
	 ...
	 *
	 * This method constructs an array that represents the call flow, utilizing the provided call_details array.
	 * Reverses the array to put the events in chronological order and adds profile end times.
	 *
	 ...
	 *
	 * This method constructs an array that represents the call flow summary using the call flow array array. The call
	 * flow summary array contains a simplified view of the call flow.
	 *
	...
	public function call_flow_summary($call_flow_array) {
		...
		if (!empty($call_flow_array)) {
			foreach ($call_flow_array as $row) {
				...
				$call_flow_summary[$x]["application_icon"]  = ["call_centers" => "fa-headset", "call_flows" => "fa-share-nodes", "conferences" => "fa-comments", "destinations" => "fa-right-to-bracket", "dialplans" => "fa-right-left", "extensions" => "fa-suitcase", "ivr_menus" => "fa-diagram-project", "ring_groups" => "fa-users", "time_conditions" => "fa-clock", "voicemails" => "fa-envelope"];
	...
	/**
	 * Finds an application in the system based on a destination array and detail action.
	 *
	 * @param array  $destination_array Destination array to search through
	 * @param string $detail_action     Detail action to match against destinations
	 *
	 * @return mixed|null Application data if found, or null otherwise
	 */
	...
	/**
	 * Reads XML files from the object property xml_cdr_dir directory and imports call detail records into the database.
	 *
	 * @return void
	 */
	...
	/**
	 * Generates a user summary report.
	 *
	 * @return array User report summary
	 */
	...
	/**
	 * Download a call recording associated with the current recording UUID in the object property recording_uuid.
	 *
	 * This method validates permissions, checks for a valid UUID, retrieves the
	 * recording path and filename from the database, and streams the file to the
	 * client with appropriate headers. Supports both binary and non-binary
	 * downloads, including HTTP range requests for non-binary audio files.
	 *
	 * @return void
	 */
	 ...
	 */
	/**
	 * Sends a range of bytes from the specified file to the client.
	 *
	...
	/**
	 * Singularize a word by removing its plural suffix.
	 *
	 * This function is based on the English language rules for forming singular nouns from their plural forms.
	 * It handles various cases such as words ending in "-es", "-s", "-z", "-sh", "-ch" and others.
	 *
	 * @param string $word The word to be made singular
	 *
