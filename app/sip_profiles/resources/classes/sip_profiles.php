...
if (!class_exists('sip_profiles')) {
	...
	class sip_profiles {
		...
		public static function dialplan_add($domain_uuid, $dialplan_uuid, $dialplan_name, $dialplan_order, $dialplan_context, $dialplan_enabled, $dialplan_description, $app_uuid) {
			//build insert array
		...
		public static function phone_letter_to_number($tmp) {
			$tmp = strtolower($tmp);
			if ($tmp == "a" | $tmp == "b" | $tmp == "c") {
				return 2;
			}
			if ($tmp == "d" | $tmp == "e" | $tmp == "f") {
				return 3;
			}
			if ($tmp == "g" | $tmp == "h" | $tmp == "i") {
				return 4;
			}
			if ($tmp == "j" | $tmp == "k" | $tmp == "l") {
				return 5;
			}
			if ($tmp == "m" | $tmp == "n" | $tmp == "o") {
				return 6;
			}
			if ($tmp == "p" | $tmp == "q" | $tmp == "r" | $tmp == "s") {
				return 7;
			}
			if ($tmp == "t" | $tmp == "u" | $tmp == "v") {
				return 8;
			}
			if ($tmp == "w" | $tmp == "x" | $tmp == "y" | $tmp == "z") {
				return 9;
			}
		}

		public static function save_call_center_xml() {
		...
		public static function xml_cdr_conf_xml() {
			//get the contents of the template
			if (file_exists('/usr/share/examples/fusionpbx/resources/templates/conf')) {
				$path = "/usr/share/examples/fusionpbx/resources/templates/conf";
			} else {
		...
		public static function save_sip_profile_xml() {
		...
		public static function save_switch_xml() {
		 ...
		 * @return void
		 */
		public static function force_close_port(string $port): void {
		...
		public static function switch_conf_xml() {
			...
			if (file_exists('/usr/share/examples/fusionpbx/resources/templates/conf')) {
				...
				$path = $document_root . "/app/switch/resources/conf";
			}
			...
			if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
				...
				$secure_path = path_join($document_root, 'secure');
				...
				$v_mailer_app = PHP_BINDIR . "/" . PHP_BIN . " " . $document_root . "/secure/v_mailto.php";
				$v_mailer_app_args = "-t";
			}
		...
		public static function get_recording_filename($id) {
			$sql = "select * from v_recordings ";
			$sql .= "where recording_uuid = :recording_uuid ";
			$sql .= "and domain_uuid = :domain_uuid ";
			$parameters['recording_uuid'] = $id;
			$parameters['domain_uuid'] = framework::domain_uuid();
			$database = framework::database();
		...
		public static function extension_presence_id($extension, $number_alias = false) {
		...
		public static function extension_exists($extension) {
		...
		public static function outbound_route_to_bridge($domain_uuid, $destination_number, array $channel_variables = null) {
		...
		public static function save_gateway_xml() {
