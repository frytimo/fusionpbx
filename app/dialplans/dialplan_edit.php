...
$language = new text()->get();

// check for unified editor redirect
$dialplan_editor_default = $_SESSION['dialplan']['editor_default']['text'] ?? 'legacy';
$dialplan_uuid_check = $_GET['id'] ?? '';
$dialplan_editor_version = '';

if (!empty($dialplan_uuid_check) && is_uuid($dialplan_uuid_check)) {
...
if (count($_POST) > 0 && empty($_POST["persistformvar"])) {
	...
	if (!empty($_POST['action']) && is_uuid($_POST['dialplan_uuid'])) {
	...
	if (empty($dialplan_order)) {
		$msg .= $text['message-required'] . $text['label-order'] . "<br>\n";
	}
	if (empty($dialplan_continue)) {
		$msg .= $text['message-required'] . $text['label-continue'] . "<br>\n";
	}
	if (empty($dialplan_context)) {
		$msg .= $text['message-required'] . $text['label-context'] . "<br>\n";
	}
	if (empty($dialplan_enabled)) {
		$msg .= $text['message-required'] . $text['label-enabled'] . "<br>\n";
	}
...
?><script language="javascript">
	...
	// show unified editor banner
	if ($dialplan_editor_default !== 'unified') {
		$unified_link = 'dialplan_edit_unified.php?id=' . urlencode($dialplan_uuid ?? '') . (!empty($app_uuid) && is_uuid($app_uuid) ? '&app_uuid=' . urlencode($app_uuid) : '');
		echo "<div style='background: #d9edf7; border: 1px solid #bce8f1; border-radius: 4px; padding: 12px 15px; margin-bottom: 15px; display: flex; align-items: center; justify-content: space-between;'>\n";
		echo "	<div><i class='fas fa-info-circle' style='color: #31708f; margin-right: 8px;'></i><span style='color: #31708f;'>" . ($text['message-unified_editor_available'] ?? 'A new visual editor is available with side-by-side XML preview and nested condition support.') . "</span></div>\n";
		echo "	<a href='" . escape($unified_link) . "' class='btn btn-sm' style='white-space: nowrap;'><i class='fas fa-external-link-alt'></i> " . ($text['button-try_new_editor'] ?? 'Try New Editor') . "</a>\n";
		echo "</div>\n";
	}
		...
		foreach ($details as $g => $group) {
			...
			if (is_array($group) && @sizeof($group) != 0) {
				foreach ($group as $index => $row) {
					...
					$dialplan_detail_uuid = $row['dialplan_detail_uuid'] ?? null;
