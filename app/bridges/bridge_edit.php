...
// get http variables from either POST or GET submitted information using safe methods
$bridge_id          = $url->get('id', '');
$bridge_uuid        = $url->get('bridge_uuid', $bridge_id);
...
$bridge_description = $url->get('bridge_description', '');

// set action
$action = is_uuid($bridge_uuid) ? "update" : "add";
...
if (($url->has_post('bridge_name') || $url->has_post('bridge_uuid')) && $url->post('persistformvar') === null) {
	...
	if (empty($bridge_name)) {
		$msg .= $text['message-required'] . " " . $text['label-bridge_name'] . "<br>\n";
	}
	// if (empty($bridge_destination)) { $msg .= $text['message-required']." ".$text['label-bridge_destination']."<br>\n"; }
	if (empty($bridge_enabled)) {
		$msg .= $text['message-required'] . " " . $text['label-bridge_enabled'] . "<br>\n";
	}
