...
//check permissions
	...
	$has_fax_queue_add    = permission_exists('fax_queue_add');
	$has_fax_queue_delete = permission_exists('fax_queue_delete');
	$has_fax_queue_update = permission_exists('fax_queue_update');
	...
	if (count($_POST) > 0 && empty($_POST["persistformvar"])) {
		...
		//process the http post data by submitted action
			if (!empty($_POST['action'])) {
				...
				switch ($_POST['action']) {
					case 'copy':
						if ($has_fax_queue_add) {
							$database->copy($array);
						}
						break;
					case 'delete':
						if ($has_fax_queue_delete) {
							$database->delete($array);
						}
						break;
					case 'toggle':
						if ($has_fax_queue_update) {
