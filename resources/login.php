					...
					if (@sizeof($results) == 1) {
						...
						if ($result['username'] != '') {
							...
							//generate reset link email and body variables
								...
								$current_time = time();
								$key = encrypt($settings->get('login', 'password_reset_key', ''), $result['username'].'|'.$result['domain_uuid'].'|'.$result['password'].'|'.$current_time);
								$reset_link = "https://".$password_reset_domain.PROJECT_PATH."/resources/login.php?action=define&key=".urlencode($key);
								$reset_button = email_button(strtoupper($text['label-reset_password']), $reset_link, ($settings->get('theme', 'button_background_color_email') ? $settings->get('theme', 'button_background_color_email') : '#2e82d0'), ($settings->get('theme', 'button_text_color_email') ? $settings->get('theme', 'button_text_color_email') : '#ffffff'));
